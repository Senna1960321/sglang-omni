...
选中文本仅在 Voice edit / Ask 所需的请求中发送到配置的文本 API。为防止写入错误位置，应用会在内存中比较目标字段的辅助功能内容、窗口和选择状态；某些编辑器的字段内容可能包含整篇文档，这些用于校验的完整字段内容不保存、不发送给模型；明确选中的编辑/问答上下文除外。不读取屏幕截图或浏览历史。辅助功能识别为密码框或系统启用安全输入时拒绝录音和插入。能读取输入框时，仍检查内容、光标和选择是否变化；微信等不暴露输入框的应用会在确认仍为同一前台应用、同一窗口后，通过 ⌘V 粘贴。此兼容路径无法检查窗口内的光标或会话变化，因此请先点进输入框，并保持焦点直到听写完成。无法确认目标窗口或校验失败时，保留结果供手动复制。不提供可靠选区信息的输入框不支持 Voice edit。远程桌面等应用仍可能不接受模拟粘贴。
...
诊断日志位于 `~/Library/Logs/OmniTyper/diagnostics.log`，设置页有入口。它按行记录 JSON 事件：失败的错误码、目标应用的 bundle ID、走的是直接写入还是粘贴、权限状态和版本。**不记录**转写内容、选中文本、输入框内容、窗口标题和文件路径 —— 这样反馈问题时可以直接附上，不必先逐行检查。文件上限 128 KiB，超出后从头按整行丢弃。
...
- **没有快捷键响应，或结果未自动输入**：先检查辅助功能权限及设置里的自动输入开关。在目标输入框中启动听写，保持焦点到处理完成；如果仍失败，查看诊断日志的 `capture.failed` / `insert.failed` 错误码。未暴露输入框的应用会使用 `paste-window` 路径；切换应用或窗口会取消自动粘贴。缺少权限时，在系统设置允许 OmniTyper 的辅助功能访问后**重启应用**。
